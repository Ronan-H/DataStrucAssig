...
> Lookup tables are generated when the cipher is initialized.
...
For encryption:
> These two packed characters, 7 bits each, are combined into one 14 bit number using binary opations << and | (14 bits can be stored in a single short).
...
> On my machine with a 3.5GHz processor, SSD storage, and DDR4 RAM, the sample input file "WarAndPeace-LeoTolstoy.txt" can be entirely read, encrypted on the fly and written in 20 milliseconds, according to the timer included in the application.
> Encrypting War and Peace times 10 completed in 120 milliseconds, so it scales very well too.
